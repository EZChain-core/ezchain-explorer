 ...
 */
export function getVersionsOnly(peerInfo) {
    return peerInfo
 ...
 * @param peerInfo
 */
export function versionsOnlyMap(peerInfo) {
    const allVersions = getVersionsOnly(peerInfo).map((v) => v.version.split('.'));
    ...
    uniqueVersions.forEach((version) => {
        const patchesForVersion = patches.filter((patch) => patch[0] === version);
        versions[version] = patchesForVersion.map((item) => item[0] + '.' + item[1]);
    });
    return versions;
}
 ...
 * @param datamap
 */
export function getVersionsColorMap(datamap) {
    const colors = [];
    const data = [];
    ...
    data.forEach((item, i) => {
        ...
        for (let j = 1; j < item[1].length; j++) {
            colors.push(chroma(categorical[i])
 ...
 */
export function getStakesOnly(peerInfo) {
    return peerInfo.filter((i) => i.stakeAmount > 0);
}
export function getTotalStake(peerInfo) {
