...
export function parseEVMBlockTxs(txs) {
    // console.log('txs                ', txs)
    if (!txs)
        return [];
    const parsedTxs = txs.map((tx) => {
        ...
        return {
            ...
            input: web3.utils.hexToAscii(tx.input),
            gasPrice: toAVAX(parseInt(web3.utils.hexToNumberString(tx.gasPrice)), 18),
...
export async function parseAtomicTxs(blockExtraData) {
    if (blockExtraData === '') {
        return [];
    }
    else {
        ...
        const buf = Buffer.from(createHash('sha256').update(blockExtraData, 'base64').digest()
            .buffer);
...
export async function parseEVMBlocks(block) {
    const parsedBlock = {
        number: parseInt(web3.utils.hexToNumberString(block.header.number)),
        timestamp: parseInt(web3.utils.hexToNumberString(block.header.timestamp)) *
            1000,
...
//# sourceMappingURL=parseEVMBlocks.js.map