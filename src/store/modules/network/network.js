...
export const DEFAULT_NETWORK_NAME = DEFAULT_NETWORK_ID === 1
    ? process.env.VUE_APP_NETWORKNAME
    : process.env.VUE_APP_TEST_NETWORKNAME;
// Mainnet
...
export const ethereumAPI_test = process.env.VUE_APP_TEST_AVALANCHE_GO_ETH_URL || '';
...
const network_module = {
    ...
    actions: {
        async init({ state, commit, dispatch }) {
            const mainnet = new Network(`${networkName} Mainnet`, `${avalancheJSProtocol}://${avalancheJSIP}:${avalancheJSPort}`, avalancheJSNetworkID, avalancheJSChainID, orteliusURL, explorerFEUrl);
            const testnet = new Network(`${networkName_test} Testnet`, `${avalancheJSProtocol_test}://${avalancheJSIP_test}:${avalancheJSPort_test}`, avalancheJSNetworkID_test, avalancheJSChainID_test, orteliusURL_test, explorerFEUrl_test);
            await commit('addNetwork', mainnet);
            await commit('addNetwork', testnet);
            // initialize selected network
            try {
                const defaultNetwork = state.networks.find((network) => network.networkId === DEFAULT_NETWORK_ID);
