...
const platform_module = {
    ...
    mutations: {
        ...
        updateChains(state, blockchains) {
            state.blockchains = blockchains;
        },
        setAnnualStakingRewardPercentage(state, APR) {
    ...
    actions: {
        ...
        async getSubnets({ state, commit }) {
            ...
            const subnets = (await platform.getSubnets()).map((s) => new Subnet(s));
            ...
            state.blockchains = (await platform.getBlockchains()).map((b) => new Blockchain(b));
        ...
        async updateMinStakeAmount({ state }) {
            state.minStake = (await platform.getMinStake(true)).minValidatorStake;
        },
        async updateAddressCounts({ state, commit }) {
            const res = await getAddressCounts();
            const updates = state.blockchains.map((chain) => {
                const toUpdate = chain;
                const addressCount = res.find((addressCount) => addressCount.chainID === toUpdate.id);
                if (addressCount) {
        ...
        async updateTxCounts({ state, commit }) {
            ...
            const updates = state.blockchains.map((chain) => {
                const toUpdate = chain;
                const txCount = res.find((txCount) => txCount.chainID === toUpdate.id);
                if (txCount) {
        ...
        async updateAnnualStakingRewardPercentage({ state, commit }) {
            ...
            const reward = calculateStakingReward(currentSupply, ONE_YEAR_SECONDS, currentSupply);
